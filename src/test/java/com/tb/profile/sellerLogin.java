...
public class sellerLogin extends TestBase {
	...
	void addRegistration(String Identity, String Password) {
		...
		httpRequest.header("userID", sellerAppID);
	...
	String[][] postProfileSellerData() throws IOException {
		...
		int rownum = XLUtils.getRowCount(path, "sellerAppLogin");
		int colcount = XLUtils.getCellCount(path, "sellerAppLogin", 1);

		String empdata[][] = new String[rownum][colcount];
		for (int i = 1; i <= rownum; i++) {
			for (int j = 0; j < colcount; j++) {
				empdata[i - 1][j] = XLUtils.getCellData(path, "sellerAppLogin", i, j);
			}
		}
