...
public class deliveryPageSignUp extends TestBase {
	@BeforeClass
	...
	void addRegistration(String first_name, String middle_name, String last_name, String gender, String mobile,
			String email, String password, String confirm_password, String group) {
		...
		//httpRequest.header("APP_ID",DeliveryAppID);

		// add the json to the body of the requestcc
		httpRequest.body(requestParams.toJSONString());

		// post request
		response = httpRequest.request(Method.POST, "auth/api/auth/create_user/delivery");
	...
	String[][] getRegData() throws IOException {
		...
		int rownum = XLUtils.getRowCount(path, "DeliveryPageSignUp");
		int colcount = XLUtils.getCellCount(path, "DeliveryPageSignUp", 1);

		String empdata[][] = new String[rownum][colcount];
		for (int i = 1; i <= rownum; i++) {
			for (int j = 0; j < colcount; j++) {
				empdata[i - 1][j] = XLUtils.getCellData(path, "DeliveryPageSignUp", i, j);
			}
		}
